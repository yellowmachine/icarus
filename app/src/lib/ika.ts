...
export async function getExposedStates(){
    ...
    const ret = s.map(w => ({...w, services: w.services.map(x => ({...x, ports: x.ports.map(y => (
        {
            ...y, 
            exposed: {
                port: `${y.exposed.port}`,
                subdomain: getSubdomain(y.mapped?.port)
            }
        }))}))}))
...
export async function cloneAndUpWorkspace(workspace: string){
    ...
    const specification = await readSpecification(p)
    const tmp = uuid()
    const tmp_name = `${workspace}_tmp_${tmp}`
    const cloned = `${rootPath}/${tmp_name}`
    await createWorkspace(cloned, readme, specification)
    return await upWorkspace(tmp_name)
}
